...
export async function POST(req: NextRequest) {
  try {
    const { priceId, productName, origin } = await req.json();

    if (!priceId || !productName || !origin) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: `${origin}/success?product_name=${encodeURIComponent(productName)}`,
      ...
      shipping_address_collection: {
        allowed_countries: [
          ...
          'NZ', 'PA', 'PE', 'PF', 'PG', 'PH', 'PK', 'PL', 'PM', 'PN', 'PR', 'PS', 'PT', 'PY', 'QA', 'RE', 
          'RO', 'RS', 'RU', 'RW', 'SA', 'SB', 'SC', 'SE', 'SG', 'SH', 'SI', 'SJ', 'SK', 'SL', 'SM', 'SN', 
